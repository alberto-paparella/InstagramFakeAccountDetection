...
def shuffle_and_split(ds_fake, ds_correct):
    ...
    ds_test = ds_fake[int(len(ds_fake) * (PERCENT_TRAIN / 100)):]
    ds_test += ds_correct[int(len(ds_correct) * (PERCENT_TRAIN / 100)):]

    random.shuffle(ds_train)
    random.shuffle(ds_test)

    df_train = pd.DataFrame.from_dict(ds_train)
    ds_test = pd.DataFrame.from_dict(ds_test)
    return df_train, ds_test
...
def get_compatible_dataset(train_df, test_df, csv):
    """
    ...
    if csv:
        ...
        custom_test_df = test_df.drop(["pic", "cl", "cz", "ni", "lt", "pr", "fo", "cs"], axis=1)
    else:
        ...
        custom_test_df = test_df.drop(["mediaLikeNumbers",
                                                   "userHasHighlighReels", "usernameLength", "usernameDigitCount",
                                                   "mediaCommentNumbers", "mediaCommentsAreDisabled",
                                                   "mediaHasLocationInfo",
                                                   "userTagsCount"],
                                     axis=1)
    return custom_train_df, custom_test_df


def get_ijece_custom_dataset(train_df, test_df):
    """
    ...
    custom_test_df = test_df.drop(["ni", "lt", "mediaHashtagNumbers"], axis=1)
    return custom_train_df, custom_test_df


def get_custom_dataset(train_df, test_df, csv):
    """
    Routing function
    :param train_df: train dataset
    :param test_df: test dataset
    ...
    """
    if csv:
        return get_ijece_custom_dataset(train_df, test_df)
    else:
        return train_df, test_df


def get_ijece_default_dataset(train_df, test_df):
    """
    ...
    custom_test_df = test_df.drop(["followerToFollowing", "hasMedia"], axis=1)
    return custom_train_df, custom_test_df


def get_default_dataset(train_df, test_df, csv):
    """
    Routing function
    :param train_df: train dataset
    :param test_df: test dataset
    ...
    """
    if csv:
        return get_ijece_default_dataset(train_df, test_df)
    else:
        return get_instafake_default_dataset(train_df, test_df)


def get_instafake_default_dataset(train_df, test_df):
    """
    ...
    default_test_df = test_df.drop(["biol",
                                                "url", "erl", "erc", "avgtime",
                                                "userHasHighlighReels", "usernameLength", "usernameDigitCount",
                                                "mediaCommentNumbers", "mediaCommentsAreDisabled",
                                                "mediaHasLocationInfo",
                                  ...
                                  axis=1)
    return default_train_df, default_test_df
...
def get_deep_learning_dataset():
    ...
    # IJECE dataset
    ijece_train, ijece_test = shuffle_and_split(fake_csv, correct_csv)
    ijece_train.to_csv(f'./dataset/deep/IJECE_df_train.csv')
    ijece_test.to_csv(f'./dataset/deep/IJECE_df_val.csv')

    # InstaFake dataset
    if_train, if_test = shuffle_and_split(fake_json, correct_json)
    if_train.to_json(f'./dataset/deep/instafake_df_train.json')
    if_test.to_json(f'./dataset/deep/instafake_df_val.json')

    # Instafake compatible dataset
    if_train_c = if_train
    if_test_c = if_test
    if_train_c, if_test_c = get_compatible_dataset(if_train_c, if_test_c, False)
    if_train_c.to_json('./dataset/deep/comp_instafake_df_train.json')
    if_test_c.to_json('./dataset/deep/comp_instafake_df_val.json')

    # IJECE compatible dataset
    ijece_train_c = ijece_train
    ijece_test_c = ijece_test
    ijece_train_c, ijece_test_c = get_compatible_dataset(ijece_train_c, ijece_test_c, True)
    ...
    partial_train, partial_test = treat_combined(partial_fake, partial_correct)
    partial_train.to_json(f'./dataset/deep/combo_partial_df_train.json')
    partial_test.to_json(f'./dataset/deep/combo_partial_df_val.json')

    # Unbalanced joined dataset
    combined_fake = fake_json + fake_csv
    combined_correct = correct_json + correct_csv
    combined_train, combined_test = treat_combined(combined_fake, combined_correct)
    combined_train.to_json(f'./dataset/deep/combo_full_df_train.json')
    combined_test.to_json(f'./dataset/deep/combo_full_df_val.json')
...
def treat_combined(fake, correct, demarcator=700):
    ...
    ijece_dataset_fake, ijece_dataset_correct = get_compatible_dataset(pd.DataFrame(data=fake[demarcator:]),
                                                                       pd.DataFrame(data=correct[demarcator:]),
                                                                       True)
    custom_train_df, custom_test_df = shuffle_and_split(

...
(ijece_train, ijece_validation) = get_compatible_dataset("ijece")
(if_train, if_validation) = get_compatible_dataset("if")
...
for i in range(eval_steps):
    loss, acc, precision, recall = ijece_model.evaluate(x=ijece_validation.iloc[:, :-1],
                                                        y=ijece_validation.iloc[:, -1], verbose=0)
...
for i in range(eval_steps):
    loss, acc, precision, recall = if_model.evaluate(x=if_validation.iloc[:, :-1],
                                                     y=if_validation.iloc[:, -1], verbose=0)

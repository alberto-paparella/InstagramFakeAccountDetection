...
(ijece_train, ijece_test) = get_compatible_dataset("ijece")
(if_train, if_test) = get_compatible_dataset("if")
...
for i in range(eval_steps):
    loss, acc, precision, recall = ijece_model.evaluate(x=ijece_test.iloc[:, :-1],
                                                        y=ijece_test.iloc[:, -1], verbose=0)
...
for i in range(eval_steps):
    loss, acc, precision, recall = if_model.evaluate(x=if_test.iloc[:, :-1],
                                                     y=if_test.iloc[:, -1], verbose=0)

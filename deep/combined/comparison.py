...
(partial_train, partial_val) = get_dataset_combined(False)
(full_train, full_val) = get_dataset_combined(True)
...
for i in range(eval_steps):
    loss, acc, precision, recall = combined_partial_model.evaluate(x=partial_val.iloc[:, :-1],
                                                                   y=partial_val.iloc[:, -1], verbose=0)
...
for i in range(eval_steps):
    loss, acc, precision, recall = combined_full_model.evaluate(x=full_val.iloc[:, :-1],
                                                                y=full_val.iloc[:, -1], verbose=0)

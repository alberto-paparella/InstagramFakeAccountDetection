...
for i in range(eval_steps):
    loss, acc, precision, recall = combined_partial_model.evaluate(x=partial_validation.iloc[:, :-1],
                                                                   y=partial_validation.iloc[:, -1], verbose=0)
...
for i in range(eval_steps):
    loss, acc, precision, recall = combined_full_model.evaluate(x=full_validation.iloc[:, :-1],
                                                                y=full_validation.iloc[:, -1], verbose=0)
